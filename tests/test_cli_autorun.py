...
def test_main_no_args_autoruns(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("screen_reader.cli.resolve_default_config_path", lambda: config_path)
    ...
    monkeypatch.setattr("screen_reader.cli.init_config", fake_init_config)
    ...
    monkeypatch.setattr("screen_reader.cli.run_command", fake_run_command)
    result = cli.main([])
    assert result == 0
