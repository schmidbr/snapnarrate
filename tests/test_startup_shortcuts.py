...
def test_shortcut_paths_have_expected_names() -> None:
    manager = ShortcutManager("Screen Reader")
    assert manager.desktop_shortcut_path().name == "Screen Reader.lnk"
    assert manager.startup_shortcut_path().name == "Screen Reader.lnk"
...
def test_startup_manager_enable_disable(tmp_path: Path) -> None:
    fake = FakeShortcutManager(tmp_path / "Screen Reader.lnk")
    manager = StartupManager(
        shortcut_manager=fake,  # type: ignore[arg-type]
        target="C:\\screen-reader.exe",
    ...
    path = manager.enable()
    assert str(path).endswith("Screen Reader.lnk")
